...
class Setting < ActiveRecord::Base
  ...
  NOTIFY_RECEIVERS = %w(author assigned_to previous_assigned watchers project_principal).freeze
...
END_SRC
  ...
  def self.load_email_issue_update_settings
    available_keys = available_settings.keys
    options_for_emails_receivers = {
      "serialized" => true,
      "default" => NOTIFY_RECEIVERS
    }
    options_for_emails_body = {
      "default" => ""
    }

    IssueStatus.all.each do |status|
      receivers_setting_name = "issue_status_#{status.id}_emails_receivers"
      emails_body_setting_name = "issue_status_#{status.id}_emails_body"

      define_setting receivers_setting_name, options_for_emails_receivers if available_keys.exclude?(receivers_setting_name)
      define_setting emails_body_setting_name, options_for_emails_body if available_keys.exclude?(emails_body_setting_name)
    end
  end

  load_available_settings
  load_plugin_settings
  load_email_issue_update_settings if IssueStatus.table_exists?
