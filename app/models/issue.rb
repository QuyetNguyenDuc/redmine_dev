...
class Issue < ActiveRecord::Base
  ...
  def ignore_notify_users
    receivers = Setting.try("issue_status_#{status_id}_emails_receivers")
    return [] unless receivers
    ignore_users = []
    ignore_users += [author] if receivers.exclude?("author")
    ignore_users += [assigned_to] if receivers.exclude?("assigned_to")
    ignore_users += [previous_assignee] if receivers.exclude?("previous_assigned")
    ignore_users += [project.notified_users] if receivers.exclude?("project_principal")
    ignore_users += [notified_watchers] if receivers.exclude?("watchers")
    ignore_users.map {|u| u.is_a?(Group) ? u.users : u}.flatten.uniq
  end
