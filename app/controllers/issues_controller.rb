...
class IssuesController < ApplicationController
  ...
  def update
    ...
    if  params[:issue][:status_id].to_i == 11
          ...
          url = 'http://10.55.123.80:8081/redmine-jbpm-intergration/api/v1/jbpm/task/status?issueid=' + @issue.id.to_s
          result = Net::HTTP.get(URI.parse(url))
          result_json = JSON.parse(result)

          if result_json["code"] == 1 && result_json["data"] != "SUCCESS" && result_json["data"] != "COMPLETED" && result_json["data"] != "COMPLETE"

            error = 'Yêu cầu đóng các task ' + result_json["data"].to_s + ' trước khi thực hiện submit'
            flash[:error] = error
