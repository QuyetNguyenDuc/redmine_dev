...
module ApplicationHelper
  ...
  def link_to_employee(user, options={})

    if user.is_a?(User)
      name = h(user.name(options[:format]))
      if user.active? || (User.current.admin? && user.logged?)
        only_path = options[:only_path].nil? ? true : options[:only_path]
          link_to name, {:controller => 'users', :action => 'show', :id => user.id}
      else
        name
      end
    else
  ...
  def link_to_user_login(user, options={})

    if user.is_a?(User)
      name = h(user.login)
      if user.active? || (User.current.admin? && user.logged?)
        only_path = options[:only_path].nil? ? true : options[:only_path]
        link_to name, {:controller => 'people', :action => 'show', :id => user.id}
      else
        name
      end
    else
  ...
  def link_to_department(department)
    if department.is_a?(Department)
      ...
      link_to(
          name,
          {:controller => 'departments', :action => 'show', :id => department.id}
      )
    else
      nil
    end
  ...
  def link_to_task_of_project(project, person)
    link_to project.name, project_issues_report_user_path(project, person.id)
  end
  ...
  def requirements_tree(requirement, options={}, &block)

    Requirement.requirements_tree(requirement, options, &block)
  ...
  def javascript_heads
    tags = javascript_include_tag('jquery-1.11.1-ui-1.11.0-ujs-5.2.3', 'application', 'responsive', 'custom')
